...
// Real 2-port S-parameter data (complex S11/S12/S21/S22), from a real
// .s2p Touchstone import -- distinct from S21Data above, which only ever
// held a single scalar magnitude ("s21") from a RigExpert-specific live
// capture command that turned out to have no confirmed hardware source
// (see CHANGELOG/dev notes around the 2-port work). Kept as a separate,
// parallel struct rather than extending S21Data so that dead-end pipeline
// stays untouched.
struct SParamPoint {
    double fq; // MHz, matching RawData.fq's convention (both come from
...
public:
    ...
    static AnalyzerParameters* byName(QString _name) {
        //{ HUCK
        //qInfo() << "In: AnalyzerParameters* byName -->  LOOKING FOR:" << _name;
    ...
    static void fill() {
        ...
        // support implemented yet, hence 0,0.

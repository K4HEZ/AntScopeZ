...
private:
    ...
    // raw RGB565 framebuffer dump (big-endian per pixel) -- no length
    // header, unlike the binary scan reply above, so the expected size is
    // just the screen dimensions. Same byte-counted approach as
    // parseBinaryScan() for the same reason (raw pixel bytes can
    // legitimately equal 0x0D/0x0A).
    //
    // These are only the *default* dimensions, used when no Custom
    // Analyzer profile (Settings > Analyzer) is active -- parseCapture()
    // prefers CustomAnalyzer::getCurrent()'s width()/height() when
    // CustomAnalyzer::customized() is set, same override
    // mainwindow_analyzer.cpp's on_actionScreenshotAA_triggered() already
    // applies to the Screenshot dialog's own sizing. That's how this app
    // covers the various NanoVNA-family screen sizes: define a profile
    // with prototype "NanoVNA" and the right LCD width/height per unit.
    //
    // Default set to 480x320 (not the 320x240 NanoVNASaver's NanoVNA.py
    // documents for the classic NanoVNA/NanoVNA-H) since that's the more
    // common size seen in practice and the one confirmed 2026-09-16
    // against a live device's Debug Logging capture -- see issue #54.
    // That mismatch (320x240 assumed, 480x320 actual) caused
    // parseCapture() to slice off only the first half of every frame,
    // reshaped against the wrong stride in Screenshot::on_newData()'s
    // NanoVNA branch: long solid-color runs (thick trace lines, divider
    // bars) survived the resulting row misalignment more or less intact;
    // fine multi-row detail (text glyphs, dotted grid lines) did not,
    // which is what made screenshots look "broken up" into scattered
    // dots while the general plot shape stayed recognizable.

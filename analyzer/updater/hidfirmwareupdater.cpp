...
HidFirmwareUpdater::FirmwareInfo HidFirmwareUpdater::firmwareInfo(const ReDeviceInfo &dev)
{
    ...
    // The FirmwareInfo payload follows a 6-byte header. Copy only as much as
    // the device actually returned: the previous fixed 60-byte copy from
    // &buff[6] read one byte past the end of buff[].
    if (ret > 6) {
        memcpy(&info, &buff[6], qMin((size_t)(ret - 6), sizeof(info)));
    }

...
{
    qInfo() << "LicenseAgent::registerDevice " ;
...
{
//    QString key = QInputDialog::getText((QWidget*)MainWindow::m_mainWindow, tr("Update license"), tr("Enter key"));
        qInfo() << "LicenseAgent::updatelicense " ;
    QString key;
...
{
    qInfo() << "LicenseAgent::showModeless";
...
{
    // qInfo() << "LicenseAgent::closemodeless";
    if (m_modelessPopup != nullptr) {
...
{
    qInfo() << "LicenseAgent::requestuserinfo";
    setState(Finished);
    requestInfo();
...
QNetworkReply* LicenseAgent::sendRequest(const QNetworkRequest& request)
{

...
void CustomGraph::drawLine(QCPPainter *painter)
{
    ...
    // Used to be gated on name() == "Graph 1", but CustomPlot::drawIncrementally()
    // always calls drawLine()/restoreLine() on graph(0) ("Graph 0") -- so this
    // bookkeeping (and restoreLine()'s erase below) was silently never running
    // for the graph it actually needs to run for. Every incoming point's XOR
    // segment got drawn and never un-drawn, leaving a permanent tick at every
    // point position. Track/erase unconditionally for whichever graph this is.

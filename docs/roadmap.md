...
| 44 | Clear all markers at once, or just the ones that no longer land on real scan data (e.g. after switching to a narrower-range measurement, a marker's original frequency falls outside it) | -- | -- | ❌ planned, not started (2026-09-06) -- right-click menu on the Markers table/chart: Clear All, plus a separate "clear markers with no data" that only removes the ones sitting outside the current measurement's scanned range, leaving valid ones alone |

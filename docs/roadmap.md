...
| 49 | Get a distributable macOS build, without an Apple Developer account | GitHub Actions (`macos-build.yml`): universal arm64+x86_64 `.app`/`.dmg`, Qt 6.11's own macOS 13+ floor | -- | ⚠️ compiles, links, launches -- confirmed via a real CI-captured screenshot on arm64 (2026-09-18); Intel (x86_64) launch smoke test in progress on `macos-15-intel`. Unsigned, not notarized (Gatekeeper needs right-click > Open or `xattr -cr`); no real analyzer hardware reachable from CI. See issue #57. |

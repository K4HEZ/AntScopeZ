...
| 9 | Retrieve scans already stored on the analyzer itself | Data from AA | Analyzer > Data from AA | ✅ |
...
| 15 | Share/document a chart as an image or PDF | Print, Save Screenshot, Analyzer > Screenshot... | File / Analyzer menus | ✅ |
...
| # | Use case | How today | Charts/tools | Status |
|---|---|---|---|---|
| 47 | Run AntScopeZ natively on Windows, not just Linux/macOS | Released in 2.2.7: `AntScopeZ-<version>-win64.exe` NSIS installer (Qt 6.11.2 MinGW), built by GitHub Actions (`windows-build.yml`) | -- | ⚠️ builds, packages and launches in CI (screenshot-verified); none of `docs/windows-port-audit.md`'s "Verification checklist" items (real analyzer over HID/FTDI, `.asd` file association, device-change notifications, per-user settings path) have been exercised against real Windows hardware yet. Unsigned installer (SmartScreen warns). See issue #58. |
| 49 | Get a distributable macOS build, without an Apple Developer account | Released in 2.2.7: `AntScopeZ-<version>-macOS.dmg`, universal arm64+x86_64 (`macos-build.yml`), Qt 6.11's own macOS 13+ floor | -- | ⚠️ builds and launches -- confirmed via real CI-captured screenshots on both arm64 and Intel (x86_64), 2026-09-18. Unsigned, not notarized (Gatekeeper needs right-click > Open or `xattr -cr`); no real analyzer hardware reachable from CI. See issue #57. |
...
| 50 | Know when a newer AntScopeZ release is out | Help > About shows "Latest Version", fetched when About opens from `version.txt` on master (Settings > Updates > "Check for updates", on by default; cached for the session, retried after a failure); no auto-download | About dialog | ✅ (shipped 2.2.7; issue #59) |

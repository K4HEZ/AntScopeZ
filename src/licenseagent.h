...
protected:
    ...
    // Issues the request via m_mng and records the resulting reply in
    // m_reply (so timeout()'s abort() and finishWaitUnitWeb()'s early-exit
    // wait loop have an actual, non-null reply to act on -- previously
    // m_reply was only ever initialized to NULL and never assigned, so both
    // of those silently did nothing, and connecting QNetworkReply::
    // errorOccurred against it in the constructor always warned "invalid
    // nullptr parameter"). Also wires up per-request error reporting, moved
    // here from the constructor since it needs a real reply to connect to.
